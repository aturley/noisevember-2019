...
actor Main
  new create(env: Env) =>
    ...
    let densities: Array[F64] = [200; 5000; 200; 120]

    let start_end_times: Array[(F64, F64)] = [(1, 3); (0, 0.05); (0.5, 1); (3, 5)]
    let dur_bands: Array[(F64, F64)] = [(0.025, 0.05); (0.01, 0.02); (0.1, 0.3); (0.3, 0.5)]
    let targetss = [[MidiToFreq(46); MidiToFreq(62); MidiToFreq(50)]; [MidiToFreq(40); MidiToFreq(44); MidiToFreq(47)]; [F64(1000)]; [F64(666); F64(1332)]]

    for (density, start_end_time, dur_band, targets) in Iter[F64](densities.values())
      .zip3[(F64, F64), (F64, F64), Array[F64]](start_end_times.values(), dur_bands.values(), targetss.values())
    do
      GlissonCloud(density, start_end_time, dur_band, targets, rand, sample_rate, l_track, r_track)
    end
